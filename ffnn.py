...
class FFNN(nn.Module):
    ...
    def forward(self, input_vector):
        ...
        output = self.W2(self.activation(hidden))
...
if __name__ == "__main__":
    ...
    with open("results.txt", "a") as f:
        ...
        f.write("Epoch    Loss    Training_accuracy    Training_time    Validation_accuracy    Validation_time    test_acc\n")
        ...
        for epoch in range(args.epochs):
            ...
            f.write("{:<20.4f}{:<17.4f}".format(correct / total, time.time() - start_time))
            ...
            f.write("{:<23.4f}{:<15.4f}".format(correct / total, time.time() - start_time))
            ...
            for input_vector, gold_label in test_data:
                predicted_vector = model(input_vector)
                predicted_label = torch.argmax(predicted_vector)
                correct += int(predicted_label == gold_label)  # Counting correct predictions
                total += 1
            print("Test accuracy for epoch {}: {}".format(epoch + 1, correct / total))
            f.write("{:<23.4f}{:<15.4f}\n".format(correct / total, time.time() - start_time))
        f.write("Total Time:{}".format(time.time() - start_time_overall))
